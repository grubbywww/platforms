...
class MysqliDb {
    ...
    function __construct($config=array()) {
        ...
        $this->prefix = $config['prefix'] ? $config['prefix'] : 'tbl_';
        if ($this->data) {
    ...
    //存储缓存
    function cache_write($key, $value,$cache_time) {
        if (!$this->dbcache || !$key || !$cache_time) {
            return false;
        }
    ...
    function get_all($sql="", $cache_time = 0, $primary="") {
        $cache_key = md5($sql);
        if ($sql && $this->dbcache) {
        ...
        $end_time = $this->time_used();
        $this->query_times += round($end_time - $start_time, 5); #[查询时间]
        $this->cache_write($cache_key, $rs, $cache_time);
        return ($rs ? $rs : false);
    }

    function get_one($sql="", $cache_time = 0) {
        $cache_key = "";
        if ($sql && $this->dbcache) {
            $cache_key = md5($sql);
            $rs = $this->cache_read($cache_key);
        ...
        $end_time = $this->time_used();
        $this->query_times += round($end_time - $start_time, 5); #[查询时间]
        $this->cache_write($cache_key, $rows, $cache_time);
    ...
    function count($sql="", $cache_time = 0) {
